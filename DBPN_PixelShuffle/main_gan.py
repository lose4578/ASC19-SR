...
parser.add_argument('--pretrained_iter', type=int, default=50, help='number of epochs to train for')
parser.add_argument('--pretrained', type=bool, default=False)
parser.add_argument('--nEpochs', type=int, default=2000, help='number of epochs to train for')
parser.add_argument('--snapshots', type=int, default=25, help='Snapshots')
parser.add_argument('--start_iter', type=int, default=199, help='Starting Epoch')
...
parser.add_argument('--patch_size', type=int, default=80, help='Size of cropped HR image')
parser.add_argument('--pretrained_sr', default='centos7DBPNLLPIRM_VGGVGG_epoch_199.pth', help='sr pretrained base model')
parser.add_argument('--load_pretrained', type=bool, default=True)
parser.add_argument('--pretrained_D', default='centos7DBPNLLPIRM_VGGVGG_epoch_Discriminator_124.pth',
                    help='sr pretrained base model')
parser.add_argument('--load_pretrained_D', type=bool, default=True)
...
writer = SummaryWriter(log_dir='tb_logger/4')
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        style_loss = opt.w4 * sum(
            [MSE_loss(utils.gram_matrix(fake_feature[i]), utils.gram_matrix(real_feature[i]).detach()) for i in
        ...
        writer.add_scalar('1/G_loss', G_loss.data, epoch * len(training_data_loader) + iteration)
        writer.add_scalar('1/D_loss', D_loss.data, epoch * len(training_data_loader) + iteration)
        writer.add_scalar('1/mse_loss', mse_loss.data, epoch * len(training_data_loader) + iteration)
        writer.add_scalar('1/perceptual', vgg_loss.data, epoch * len(training_data_loader) + iteration)
        writer.add_scalar('1/style_loss', style_loss.data, epoch * len(training_data_loader) + iteration)
        writer.add_scalar('1/adv', GAN_loss.data, epoch * len(training_data_loader) + iteration)
        print("Epoch: [%2d] [%4d/%4d] G_loss: %.8f, D_loss: %.8f, mse:%.8f, perceptual: %.8f, style: %.8f, adv: %.8f"
              % (
                  (epoch), (iteration), len(training_data_loader), G_loss.data, D_loss.data, mse_loss.data,
                  vgg_loss.data,
                  style_loss.data, GAN_loss.data))
    ...
    print(
        "===> Epoch {} Complete: Avg. Loss G: {:.4f} D: {:.4f} MSE: {:.4f} Perceptual: {:.4f} Style: {:.4f} Adv: {:.4f}".format(
            epoch, G_epoch_loss / len(training_data_loader), D_epoch_loss / len(training_data_loader),
                   mse_epoch_loss / len(training_data_loader), feat_epoch_loss / len(training_data_loader),
                   style_epoch_loss / len(training_data_loader), adv_epoch_loss / len(training_data_loader)))

    writer.add_scalar('Avg/Loss G', G_epoch_loss / len(training_data_loader),
                      epoch * len(training_data_loader) + iteration)
    writer.add_scalar('Avg/D_epoch_loss', D_epoch_loss / len(training_data_loader),
                      epoch * len(training_data_loader) + iteration)
    writer.add_scalar('Avg/mse_epoch_loss', mse_epoch_loss / len(training_data_loader),
                      epoch * len(training_data_loader) + iteration)
    writer.add_scalar('Avg/feat_epoch_loss', feat_epoch_loss / len(training_data_loader),
                      epoch * len(training_data_loader) + iteration)
    writer.add_scalar('Avg/style_epoch_loss', style_epoch_loss / len(training_data_loader),
                      epoch * len(training_data_loader) + iteration)
    writer.add_scalar('Avg/adv_epoch_loss', adv_epoch_loss / len(training_data_loader),
                      epoch * len(training_data_loader) + iteration)
...
def checkpoint(epoch, pretrained_flag=False):
    if pretrained_flag:
        ...
        model_out_path = opt.save_folder + hostname + opt.model_type + opt.prefix + opt.feature_extractor + "_epoch_{}.pth".format(
            epoch)
        model_out_path_D = opt.save_folder + hostname + opt.model_type + opt.prefix + opt.feature_extractor + "_epoch_Discriminator_{}.pth".format(
            epoch)
