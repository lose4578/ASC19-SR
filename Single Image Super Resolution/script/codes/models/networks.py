...
def define_G(opt):
    ...
    elif which_model == 'RRDBx13_DPUSx5':  # RRDBx13_DPUSx5
        netG = arch.RRDBx13_DPUSx5(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'], nf=opt_net['nf'],
            nb=opt_net['nb'], gc=opt_net['gc'], upscale=opt_net['scale'], norm_type=opt_net['norm_type'],
            act_type='leakyrelu', mode=opt_net['mode'], upsample_mode='upconv')

    elif which_model == 'DPUSx10':  # DPUSx10
        netG = arch.DPUSx10(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'], nf=opt_net['nf'],
            nb=opt_net['nb'], gc=opt_net['gc'], upscale=opt_net['scale'], norm_type=opt_net['norm_type'],
            act_type='leakyrelu', mode=opt_net['mode'], upsample_mode='upconv')

    elif which_model == 'RRDBNet_WN':  # RRDBNet_WN
        netG = arch.RRDBNet_WN(in_nc=opt_net['in_nc'], out_nc=opt_net['out_nc'], nf=opt_net['nf'],
            nb=opt_net['nb'], gc=opt_net['gc'], upscale=opt_net['scale'], norm_type=opt_net['norm_type'],
            act_type='leakyrelu', mode=opt_net['mode'], upsample_mode='upconv')
    else:
    ...
    # data = torch.Tensor(1, 3, 40, 40)
    # tb_logger = SummaryWriter(log_dir='../tb_logger/' + opt['name'])
    # tb_logger.add_graph(netG.cuda(), input_to_model=data)
