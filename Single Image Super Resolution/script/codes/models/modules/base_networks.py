...
# 改 ##########################################################################
class ConvBlock_1(torch.nn.Module):
...
def ConvBlock(input_size, output_size, kernel_size=3, stride=1, padding=1, bias=True, activation='prelu',
              norm=None):
    conv = torch.nn.Conv2d(input_size, output_size, kernel_size, stride, padding, bias=bias)

    norm = norm
    if norm == 'batch':
        bn = torch.nn.BatchNorm2d(output_size)
    elif norm == 'instance':
        bn = torch.nn.InstanceNorm2d(output_size)
